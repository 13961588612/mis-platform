 ...
 * </ul>
 *
 * <p>type-to-confirm（输入库名完全一致）只对物理删除是「防手滑」，对归档同样保留——
 * 归档会让库从检索里消失，误操作的业务影响与删除同级。
 */
export function KbLibraryDeleteDialog({
  ...
  async function onSubmit() {
    ...
    setSubmitting(true);
    try {
      const res = await deleteLibrary(library.id, mode);
      setResult(res);
      ...
      <DialogContent className="max-w-xl">
        ...
        <DialogFooter>
          {result ? (
            ...
            <>
              <Button
                variant={mode === 'physical' ? 'destructive' : 'default'}
                disabled={!confirmed || submitting}
                onClick={() => void onSubmit()}
              >
                {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
                {mode === 'archive' ? '确认归档' : '确认物理删除'}
              </Button>
