...
export function PostTypeManagePage({ headerExtra }: { headerExtra?: ReactNode }) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <PageHeader
        title="岗位类型管理"
        description="岗位类型层级树：默认仅顶级；是否末级为显式字段；仅非末级（分类）可挂子类型。"
      ...
      <div className="min-h-0 flex-1 overflow-auto rounded-lg border bg-table-surface">
        {loading ? (
          ...
          <TreeTable
            ...
            storageKey="mis-post-type-tree-table-widths-v2"
            rowIcon={(row) => {
        ...
        <SheetContent className="flex w-full flex-col sm:max-w-md">
          ...
          <div className={SHEET_FORM_BODY}>
            ...
            <div className={SHEET_FORM_FIELD}>
              ...
              >
                <option value={1}>末级（可被岗位选用）</option>
                <option value={0}>非末级 / 分类（可挂子类型）</option>
              </select>
              <p className="mt-1 text-xs text-muted-foreground">
                显式字段：有子类型时不可改为末级；已被岗位引用时不可改为分类。
              </p>
            </div>
            <div className={SHEET_FORM_FIELD}>
