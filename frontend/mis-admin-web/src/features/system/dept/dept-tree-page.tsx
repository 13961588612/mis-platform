...
import { ChevronRight, Folder, Network, Pencil, Plus, Trash2, Users } from 'lucide-react';
...
type DeptRow = { id: string; depth: number; node: DeptNode };
...
export function DeptTreePage() {
  ...
  // E.1 三视图合一：行内展开「岗位编制」面板（懒加载 fetchDeptStaffing），关闭即收起
  ...
  /** 行内展开/收起岗位编制面板；展开时懒加载该部门真实编制（GET /depts/{id}/staffing）。 */
  ...
  /** 行内「穿透下钻」：以当前部门为锚点打开 OrgPierceDrawer。 */
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <PageHeader
        title="部门管理"
        description="组织内部门树（三视图合一）：行内展开岗位编制、行内穿透下钻；对应组织锚点驱动只读穿透。"
          ...
          <tbody>
            {loading ? (
              ...
              rows.map((row) => {
                ...
                const vo = staffingMap[row.id];
                const staffingLoading = staffingLoadingId === row.id;
                return (
                  <Fragment key={row.id}>
                    <tr className="border-b border-border/50 bg-table-row even:bg-table-stripe hover:bg-table-hover">
                      {/* 部门名称（树）+ 行内展开编制 */}
                      <td className="overflow-hidden whitespace-nowrap px-3 py-2 align-middle" style={{ paddingLeft: 12 + row.depth * 16 }}>
                        <span className="inline-flex items-center gap-1.5">
                          <button
                            type="button"
                            onClick={() => toggleStaffing(row.id)}
                            className="inline-flex h-5 w-5 items-center justify-center rounded text-muted-foreground hover:bg-muted"
                            aria-label={expanded ? '收起岗位编制' : '展开岗位编制'}
                      ...
                      <td className="overflow-hidden whitespace-nowrap px-3 py-2 align-middle">
                        {!row.node.linkedOrgId ? (
                          ...
                          <span className="inline-flex items-center rounded-md bg-muted px-2 py-0.5 text-xs text-foreground/80">
                            {row.node.linkedOrgName || orgNameOf(row.node.linkedOrgId) || row.node.linkedOrgId}
                          </span>
                        )}
                      </td>
                      ...
                      <td className="overflow-hidden whitespace-nowrap px-3 py-2 align-middle text-center">{row.node.sort ?? 0}</td>
                      <td className="overflow-hidden whitespace-nowrap px-3 py-2 align-middle">
                        <StatusBadge
                      ...
                      <td className="px-3 py-2 align-middle">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button
                            type="button"
                            className="inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[0.8125rem] text-primary hover:bg-primary/10"
                            onClick={() => openPierce(row.node)}
                            title={row.node.linkedOrgId ? '穿透下钻' : '该部门未配置对应组织'}
                          >
