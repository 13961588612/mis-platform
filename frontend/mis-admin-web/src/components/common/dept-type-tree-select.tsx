...
export interface DeptTypeTreeSelectProps {
  /** 当前选中部门类型 id（单值）；回填时由引擎传入 */
...
/**
 * 部门类型下拉树（BFF GET /dept-types/tree）。
 *
 * <p>默认仅末级可选；上级类型选择用 {@code selectMode='non-leaf'}。
 */
export function DeptTypeTreeSelect({
  value,
  onChange,
  placeholder,
  disabled,
  selectMode,
  excludeId = null,
}: DeptTypeTreeSelectProps) {
  const mode: 'leaf' | 'non-leaf' | 'any' = selectMode ?? 'leaf';
  ...
  const idToName = useMemo(() => {
    ...
    const walk = (nodes: DeptTypeTreeNode[]) => {
  ...
  const excludedIds = useMemo(() => {
    ...
    set.add(selfKey);
    const byId = new Map<string, DeptTypeTreeNode>();
    const walkIndex = (nodes: DeptTypeTreeNode[]) => {
      for (const n of nodes) {
        byId.set(n.id, n);
    ...
    walkIndex(tree);
    const queue: DeptTypeTreeNode[] = [byId.get(selfKey)].filter(Boolean) as DeptTypeTreeNode[];
    while (queue.length) {
  ...
  const isSelectable = (n: DeptTypeTreeNode) => {
  ...
  const renderNodes = (nodes: DeptTypeTreeNode[], depth: number): ReactNode =>
    nodes.map((n) => {
    ...
    <Popover modal open={open} onOpenChange={(o) => !disabled && setOpen(o)}>
      ...
      <PopoverContent className="w-80" align="start">
        <div className="max-h-72 overflow-auto rounded-md border border-border/60 p-1">
          ...
          {tree.length === 0 ? (
            <div className="px-2 py-3 text-center text-xs text-muted-foreground">暂无部门类型</div>
          ) : (
            renderNodes(tree, 0)
          )}
        </div>
