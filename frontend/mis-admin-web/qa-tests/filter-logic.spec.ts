 ...
 */
import { assertDeepEqual, assertEqual, describe, it } from './harness';
...
// ---------------------------------------------------------------------------
function serverFilterSignature(def: Def, applied: Record<string, unknown>): string {
...
describe('admin-list-page filtered —— serverFilterKeys 跳过客户端二次过滤', () => {
  it('deptIds 已选但行上没有 deptIds 字段 → 不得把结果过滤空（核心回归点）', () => {
    const out = applyClientFilter(ROWS, POST_DEF, { deptIds: ['101', '102'] });

    assertEqual(out.length, 3, '服务端已过滤的 key 必须跳过，否则数组匹配 undefined 会全空');
  });

  it('orgIds 已选 → 同样跳过，不影响结果', () => {
    const out = applyClientFilter(ROWS, POST_DEF, { orgIds: ['10', '20'] });

    assertEqual(out.length, 3);
  });

  it('deptIds + orgIds 同时选 → 仍全量保留（交集由后端完成）', () => {
    const out = applyClientFilter(ROWS, POST_DEF, { deptIds: ['101'], orgIds: ['10'] });

    assertEqual(out.length, 3);
  });

  it('客户端 key 仍正常过滤：name 模糊匹配', () => {
  ...
  it('客户端 key 仍正常过滤：status 精确匹配（select）', () => {
  ...
  it('服务端 key 与客户端 key 叠加：deptIds 跳过、name 生效', () => {
    const out = applyClientFilter(ROWS, POST_DEF, { deptIds: ['999'], name: '研发' });

    assertDeepEqual(
      out.map((r) => r.id),
      ['1'],
      'deptIds 不参与客户端过滤，name 仍须生效',
    );
  });

  it('未配置 serverFilterKeys 时，multiselect 走数组包含过滤', () => {
    const def: Def = { filters: [{ key: 'deptId', type: 'multiselect' }] };
    ...
    assertDeepEqual(
      out.map((r) => r.id),
      ['1', '3'],
      'multiselect 应按「单元格值 ∈ 已选集合」过滤',
    );
  });

  it('multiselect 空数组 → 不约束（返回全部）', () => {
    const def: Def = { filters: [{ key: 'deptId', type: 'multiselect' }] };

    const out = applyClientFilter(ROWS, def, { deptId: [] });

    assertEqual(out.length, 3, '空数组必须视作不约束');
  });

  it('multiselect 数字与字符串混用可比对（选项 value 可能是 number）', () => {
    const def: Def = { filters: [{ key: 'deptId', type: 'multiselect' }] };
    ...
    assertDeepEqual(
      out.map((r) => r.id),
      ['1', '2'],
      '须经 String() 归一后比较，否则数字选项匹配不到字符串单元格',
    );
  });
});

describe('admin-list-page serverFilterSignature —— 仅服务端筛选键变化才重拉数据', () => {
  it('deptIds 数组变化 → 签名变化（触发 loader 重拉）', () => {
    const before = serverFilterSignature(POST_DEF, { deptIds: ['1'] });
    const after = serverFilterSignature(POST_DEF, { deptIds: ['1', '2'] });

    assertEqual(before === after, false, '服务端筛选键变化必须改变签名');
  });

  it('仅客户端键（name）变化 → 签名不变（不触发无谓网络请求）', () => {
    const before = serverFilterSignature(POST_DEF, { deptIds: ['1'], name: 'a' });
    const after = serverFilterSignature(POST_DEF, { deptIds: ['1'], name: 'b' });

    assertEqual(before, after, '客户端筛选不应触发 loader 重拉');
  });
  ...
  it('orgIds 变化 → 签名变化', () => {
    ...
    assertEqual(before === after, false);
  });

  it('清空 deptIds（[] → 未传）签名一致，均为空串', () => {
    assertEqual(
      serverFilterSignature(POST_DEF, { deptIds: [] }),
      serverFilterSignature(POST_DEF, {}),
      '空数组与未传应产生相同签名，避免重复拉取',
    );
  });

  it('两个服务端键以 | 分隔，互不串味（["1","2"]+[] ≠ ["1"]+["2"]）', () => {
    const a = serverFilterSignature(POST_DEF, { deptIds: ['1', '2'], orgIds: [] });
    const b = serverFilterSignature(POST_DEF, { deptIds: ['1'], orgIds: ['2'] });

    assertEqual(a === b, false, '分隔符必须防止不同组合产生相同签名');
    assertEqual(a, '1,2|');
    assertEqual(b, '1|2');
  });
});
