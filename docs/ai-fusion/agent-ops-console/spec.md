...
> 状态：✅ 规范已发布（O0）｜版本：v1.2｜日期：2026-08-04  
> 决策依据：[adr.md](adr.md)  
> 产品需求：[prd.md](prd.md)  
> 界面设计（强制）：[ui.md](ui.md)  
...
- 控制台 UI **完整覆盖** [ui.md](ui.md) §0 **十项**强制能力（含 **#10 Coordinator–Worker 调度配置**）。
- 复用并扩展 `/agents`、`/skills`、`/mcp`、`/sessions`、Gateway 企微、ConfigManager。
- Agent 级 C–W 配置与全局 WorkerCatalog / 委派白名单与 [C–W Spec](../coordinator-worker/spec.md) 字段对齐。

### 1.2 非目标

- MIS host App / 业务多 Agent 选择器
- 无白名单的任意文件系统浏览
- 新建 Java `mis-agent` 服务
- 在控制台实现完整 DAG 工作流引擎（C–W 非目标）

---

## 2. 前端路由与导航（必须）

权威线框与侧栏分组见 [ui.md](ui.md)。路由表：

| 常量建议 | 路径 | 页面 | UI# | 状态 |
|----------|------|------|-----|------|
| `CHAT` | `/chat` | ChatPage（运营调试） | #6 | 已有，补文案 |
| `ADMIN_SESSIONS` | `/admin/sessions` | SessionsPage | #4 | **新增** |
| `ADMIN_AGENTS` | `/admin/agents` | AgentManagePage | — | 取消 redirect |
| — | `/admin/agents/:id` | AgentDetailShell | — | 新增 |
| `ADMIN_AGENT_SKILLS` | `/admin/agents/:id/skills` | AgentSkillsPage | #5 | **新增** |
| `ADMIN_AGENT_CONFIG` | `/admin/agents/:id/config` | AgentConfigPage | #9 | **新增** |
| `ADMIN_AGENT_CW` | `/admin/agents/:id/coordination` | AgentCoordinationPage | **#10** | **新增** |
| `ADMIN_CATALOG` | `/admin/catalog` | CatalogPage | **#10** | 新增（C–W 全局） |
| `ADMIN_DISPATCH` | `/admin/dispatch` | DispatchPage | — | 新增（C–W） |
| `ADMIN_SKILLS` | `/admin/skills` | SkillManagePage | #1 #7 | 已有，补齐创建/删/停用 UX |
| `ADMIN_SKILL_ACL` | `/admin/skills/permissions` | SkillPermissionsPage | #2 | **新增** |
| `ADMIN_MCP` | `/admin/mcp` | McpManagePage | #8 | **新增** |
| `ADMIN_WECOM` | `/admin/channels/wecom` | WecomBotsPage | #3 | **新增** |
| `ADMIN_MONITOR` | `/admin/monitor` | MonitorPage | — | 已有 |
| `ADMIN_APPROVALS` | `/admin/approvals` | ApprovalCenterPage | — | 已有 |
| `ADMIN_USERS` | `/admin/users` | UsersPage | — | 占位→实现 |

侧栏必须按 ui.md §2 分组展示；**缺任一 UI#（含 #10）主入口则界面验收失败。**

---

## 3. API 契约

统一响应：`{ code, data, message, traceId }`。前缀 `/api/v1`。鉴权：Bearer JWT。
...
### 3.2 技能权限 ACL（UI#2）— 新建

| 方法 | 路径 | 用途 |
|------|------|------|
| GET | `/skills/acl` | 列表（可按 skill_id 过滤） |
| PUT | `/skills/acl` | 全量或增量替换某 skill 的授权集 |
| POST | `/skills/acl/entries` | 新增一条 |
| DELETE | `/skills/acl/entries/{id}` | 删除一条 |
| POST | `/skills/acl/check` | 调试：给定 subject 是否可运行 |
...
**默认策略（锁定）：** fail-closed——无显式 allow 则不可执行该 Skill（平台 `admin` 角色豁免）。`deny` 优先于 `allow`。

运行时：Skill 工具执行前调用 ACL；拒绝时返回可理解错误码。
...
| 阶段 | 内容 | UI# | 验收 |
|------|------|-----|------|
| O0 | 文档含 ui.md | 全部写入 | 对照表齐全 |
| O1a | 技能池 UX + Agent 总览 + Chat 文案 | #1 #6 #7 | B1/B2/B9 |
| O1b | 会话列表/删 | #4 | B5 |
| O1c | MCP 页 | #8 | B8 |
| O1d | 绑技能 + 配置编辑 | #5 #9 | B6/B7 |
| **O1g** | **Coordination 页 + Catalog 页** | **#10** | B9/B10；可与 C2–C3 并行 |
| O1e | Skill ACL | #2 | B3 |
| O1f | 多企微 Bot | #3 | B4 |
| O2 | Dispatch | — | C1 |
| O3 | Catalog↔工具 schema 同步 | #10 增强 | C3 |
