...
> 文档角色：架构师（高见远）给出的**前端集成设计 + 任务分解**，承接 PRD `phase5-frontend-prd.md`、身份文档 `../decisions/identity-jwt.md` / `../archive/identity-enrichment-task-list.md`。
...
- **身份/权限一致**：`AIProvider` 消费 `/features` 的 `allowedCategories`/`canApprove`，与 `../decisions/identity-jwt.md` / `../archive/identity-enrichment-task-list.md` 的 `UserContext` 演进一致；前端不本地算 category。
- **写操作铁律**：AI 仅生成建议/抽取/摘要；回填为"建议值"，用户 HITL 确认后经原领域 API（本期领域为 `AdminListPage` 本地 state）落值，AI 层不承载交易写。

## 附录 B：引用文件（已读）

- `phase5-frontend-prd.md`（主输入）
- `../decisions/identity-jwt.md`、`../archive/identity-enrichment-task-list.md`
