...
> 文档角色：产品经理（许清楚）产出的**增量 PRD**，承接《阶段5 前端 AI 融合 PRD》（`phase5-frontend-prd.md`）与《阶段5 集成设计》（`phase5-frontend-design.md` §0.2/§4.3/§7.1/§8）、《后端集成契约审计》（`phase5-backend-audit.md` §6/§7）。
...
- 上游 PRD：`phase5-frontend-prd.md`（阶段5 主 PRD，UC-1~UC-6、§5 字段映射、§8 待确认）
- 集成设计：`phase5-frontend-design.md` §0.2（契约实测）、§4.3（T-ext-1/2/T-sum/T-stream 四行）、§7.1（任务总表）、§8（待拍板清单 Q1~Q11）
- 契约审计：`phase5-backend-audit.md` §6（错误码/401 透传）、§7（SSE 缺口）、附 file:line 索引
