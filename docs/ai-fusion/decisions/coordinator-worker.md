...
> 详细契约：[../specs/coordinator-worker-spec.md](../specs/coordinator-worker-spec.md)
...
2. **Coordinator ID 保持 `mis-copilot`**（BFF `capability=chat` 映射不变）；产品展示名可为「MIS 智能助手」。
...
- 并行 spawn / send_message 续聊分期落地，首版以串行综合为主

## 关联

- 规范：`docs/ai-fusion/specs/coordinator-worker-spec.md`
