# ADR-008: API 权限校验 — BFF 统一拦截，mis-rbac 作策略中心
...
| 角色 | 组件 | 职责 |
|------|------|------|
| **PDP**（策略决策点，Policy Decision Point） | **mis-rbac** | 权限数据权威来源：角色、菜单、权限聚合；提供内部查询 API；Redis 缓存 |
| **PEP**（策略执行点） | **mis-admin-bff** | **唯一**对外 API 权限校验（`ApiPermissionInterceptor` + 映射表，见 ADR-010） |
| **认证** | mis-gateway + mis-auth | 仅认证，不做接口权限 |
| **数据权限** | 各领域服务 | `@DataScope`（行级），与 API 权限分离 |
...
### mis-rbac 内部 API（供登录与 BFF 可选调用）

```
GET  /internal/v1/permissions/{userId}     # 登录/refresh 加载并写 Redis；BFF 缓存 miss 时回源
```

- **登录/refresh**：mis-auth 调 rbac → **permissions 写入 Redis**，JWT **不含** permissions
...
| B. **BFF 统一 PEP + rbac PDP（选定）** | 对外单点校验，职责清晰 |
...
### 正面
- 对外 API 权限**只维护一份**（BFF Controller）
- mis-rbac 专注权限模型与数据，不扛每秒万级 check QPS
- 领域服务代码更简，专注业务与数据权限
