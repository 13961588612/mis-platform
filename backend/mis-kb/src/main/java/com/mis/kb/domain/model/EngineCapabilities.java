...
public record EngineCapabilities(
    ...
    /**
     * 兼容构造：6 参数旧签名，OCR/overlap/graph 三个布尔位置 {@code false}（不支持）。
     *
     * <p>新增布尔位后既有 6 参构造点（适配器、测试夹具）保持零改动；
     * 需要显式声明 parser/图谱能力时请用 {@link #of(boolean, boolean, boolean, boolean, boolean, boolean, boolean, boolean)}。
     */
    public EngineCapabilities(
        ...
        this(capabilities, rerankSupported, metadataFilterSupported, replaceSupported,
                hybridSupported, deleteSupported, false, false, false);
    }

    /**
     * 全不支持的能力声明（noop 引擎、能力探测失败时使用）。
     *
     * @return 八个布尔位全 false 的声明
     */
    public static EngineCapabilities unsupported() {
        return new EngineCapabilities(
                List.of(UNSUPPORTED), false, false, false, false, false, false, false, false);
    }

    /**
     * 由五个布尔位反推 {@code capabilities} 列表（OCR/overlap/graph 默认不支持）。
     *
     * <p>保持既有 5 参调用点零改动（parser/graph 恒为 false）。
     *
     * @param rerank         重排是否可用
    ...
    public static EngineCapabilities of(
        ...
        return of(rerank, metadataFilter, replace, hybrid, delete, false, false, false);
    }

    /**
     * 由七个布尔位反推 {@code capabilities} 列表（graph 默认不支持）。
     *
     * <p>保持既有 7 参调用点零改动（如 {@code MockAdapter}，图谱能力恒 false）。
     *
     * @param rerank         重排是否可用
     * @param metadataFilter 元数据过滤是否可用
     * @param replace        同名替换是否可用
    ...
    public static EngineCapabilities of(
        ...
        return of(rerank, metadataFilter, replace, hybrid, delete, parserOcr, parserOverlap, false);
    }

    /**
     * 由八个布尔位反推 {@code capabilities} 列表，避免各适配器手工维护两份信息导致漂移。
     *
     * @param rerank         重排是否可用
     * @param metadataFilter 元数据过滤是否可用
     * @param replace        同名替换是否可用
    ...
    public static EngineCapabilities of(
        ...
        List<String> caps = new ArrayList<>(8);
        if (hybrid) {
            caps.add(CAP_HYBRID);
        }
        if (rerank) {
